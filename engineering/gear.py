...
def generate_tooth_profile(
    module: float,
    teeth: int,
    pressure_angle: float = 20.0,
    n_points_per_flank: int = 40,
) -> list[tuple[float, float]]:
    """Single tooth profile: right flank, tip arc, mirrored left flank."""
    ...
    # Tooth half-thickness angle on the pitch circle.
    s = math.pi * module / 2.0
    theta_pitch = s / pitch_r / 2.0

    # Find involute parameter at pitch circle so we can shift the flank to be
    # symmetric about the tooth centerline (y=0 axis after rotation).
    if outer_r > base_r:
        t_pitch = math.sqrt((pitch_r / base_r) ** 2 - 1.0) if pitch_r > base_r else 0.0
    else:
        t_pitch = 0.0
    inv_pitch = math.tan(pa) - pa  # involute function at pressure angle
    # The flank generated by `generate_involute_flank` starts at theta=0 on the
    # base circle. The involute point at the pitch radius makes angle inv_pitch
    # (approx). We rotate the flank by -inv_pitch + theta_pitch so the pitch
    # crossing sits at +theta_pitch above the tooth-center axis.
    rot = -inv_pitch + theta_pitch

    right_flank = generate_involute_flank(base_r, root_r, outer_r, n_points_per_flank)
    right_flank = _rotate_points(right_flank, rot)

    # Left flank by mirroring across the X axis (tooth centerline).
    left_flank = [(x, -y) for x, y in right_flank]

    # Tip arc connects the outermost right-flank point to the outermost left-flank point.
    tip_right = right_flank[-1]
    tip_left = left_flank[-1]
    a_right = math.atan2(tip_right[1], tip_right[0])
    a_left = math.atan2(tip_left[1], tip_left[0])
    if a_left > a_right:
        a_left -= 2 * math.pi
    tip_arc = _arc_points(0.0, 0.0, outer_r, a_right, a_left, n=5)

    profile: list[tuple[float, float]] = []
    profile.extend(right_flank)
    profile.extend(tip_arc[1:-1])
    profile.extend(reversed(left_flank))
    return profile
